...
- The same script now also runs on `PreToolUse`, where it records the `HEAD` of every repository the command names that the session has not met before. `SessionStart` covers the repositories on the watch set when the session opens, which is the session's own and nothing else; a hint adds more as the session goes, and a repository added that way was never baselined — a read-only `git -C ../other status` is enough to put one there, and whatever sits at its `HEAD` arrives unaccounted for without having moved at all. Nothing observed after the command can separate that from a commit: a commit made in that repository ten minutes ago by a second agent or a terminal alongside looks exactly like one made by the call that just ran. Running the same hints *before* the command settles it, because at that point `HEAD` is still where the command found it, which is what makes the question the hook asks afterwards — did `HEAD` move? — answerable at all for a repository being met mid-session. Only newly met repositories are recorded, so a commit made between two tool calls is still a `HEAD` that moved while the hook was watching, and still reported. Nothing is written to stdout on that path, since a `PreToolUse` hook's output is how a host is told to block a tool call
- A repository that reaches `PostToolUse` still unmet — on a host that does not run `PreToolUse`, or with the hook added to a session already under way — is recorded rather than reported. Its `HEAD` was never observed to move, only found somewhere. The cost is a commit in a repository the session had never touched until the call that committed in it; the call after that one is detected normally, so the degradation lasts one call rather than the session
- The watch set is bounded on both sides when tested for membership. `/repo/.git` read as already present in a set holding only `/other/repo/.git`, so a repository whose git directory was a path suffix of one already watched was dropped and its commits went unreported
