...
| **Repos met mid-session** | A hint adds a repository however the command was using it, so `git -C ../other status` puts one on the watch set that was never baselined — and whatever sits at its `HEAD` arrives unaccounted for without having moved. The session-wide date window is far too loose to tell that from a commit: anything committed there since the session opened, by a second agent or a terminal alongside, would read as yours. A repository met for the first time is therefore baselined on sight, and nudged about only if its `HEAD` moved within a couple of minutes of the tool call that just ran — so `git -C ../other commit` is caught exactly as before, while looking at a repository reports what was already there rather than announcing it |
