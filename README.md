...
| **Repos met mid-session** | `SessionStart` baselines the session's own repo and nothing else, but a hint adds repos as the session goes — `git -C ../other status` is enough — and one added that way has a `HEAD` that's unaccounted for without having moved at all. Nothing observed afterwards separates that from a commit: work another agent committed there ten minutes ago looks exactly like work the last call did. So the same script also runs on `PreToolUse`, where the command hasn't run yet and `HEAD` is still where the command found it, and records the repos it's meeting for the first time. One that reaches `PostToolUse` still unmet was baselined by neither, and is recorded rather than announced |
